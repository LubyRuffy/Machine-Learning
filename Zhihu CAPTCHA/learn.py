...
logger = logging.getLogger('Training Zhihu CAPTCHA recognition')
...
tf.app.flags.DEFINE_integer('image_width', 280,
                            "the width of image ")
tf.app.flags.DEFINE_integer('image_height', 30, 'the height of image')
...
tf.app.flags.DEFINE_string('train_data_dir', './train_data', 'the train dataset dir')
tf.app.flags.DEFINE_string('test_data_dir', './test_data', 'the test dataset dir')
tf.app.flags.DEFINE_boolean('restore', False, 'whether to restore from checkpoint')
tf.app.flags.DEFINE_boolean('batch_size', 100, 'the batch size during train and val')
...
class DataIterator:
    def __init__(self, data_dir):
        ...
        self.labels = [self.get_label(file_name.split('/')[-1].split('.')[0].split('_')[-1]) for file_name in
                       self.image_names]
    ...
    def get_label(self, str_label):
        """
        Convert the str_label to  7-bit binary code
        """
        result = [0] * 7
        for i in str_label:
            result[int(i)] = 1
...
#            images = tf.image.resize_images(images,[30,280])
#        return images
    ...
    def input_pipeline(self, batch_size, num_epochs=None, aug=False):
        ...
        image_batch, label_batch = tf.train.shuffle_batch([images, labels], batch_size=batch_size, capacity=50000, min_after_dequeue=1000)
        return image_batch, label_batch


def network():
    images = tf.placeholder(dtype=tf.float32, shape=[None, 30, 280, 1], name='image_batch')
    labels = tf.placeholder(dtype=tf.int32, shape=[None, 7], name='label_batch')
    endpoints = {}

    conv_1 = slim.conv2d(images, 32, [5, 5], 1, padding='SAME')
    max_pool_1 = slim.max_pool2d(conv_1, [2, 2], [1, 1], padding='SAME')

    conv_2 = slim.conv2d(max_pool_1, 32, [5, 5], 1, padding='SAME')
    max_pool_2 = slim.max_pool2d(conv_2, [2, 2], [1, 1], padding='SAME')

    conv_3 = slim.conv2d(max_pool_2, 32, [5, 5], 1, padding='SAME')
    max_pool_3 = slim.max_pool2d(conv_3, [2, 2], [1, 1], padding='SAME')
    ...
    out_score = tf.concat([out0, out1, out2, out3, out4, out5, out6], axis=1)
    out_final = tf.cast(tf.concat(
        [out0_argmax, out1_argmax, out2_argmax, out3_argmax, out4_argmax, out5_argmax, out6_argmax], axis=1), tf.int32)

    loss = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out0, labels=tf.one_hot(labels[:, 0], depth=2)))
    loss1 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out1, labels=tf.one_hot(labels[:, 1], depth=2)))
    loss2 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out2, labels=tf.one_hot(labels[:, 2], depth=2)))
    loss3 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out3, labels=tf.one_hot(labels[:, 3], depth=2)))
    loss4 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out4, labels=tf.one_hot(labels[:, 4], depth=2)))
    loss5 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out5, labels=tf.one_hot(labels[:, 5], depth=2)))
    loss6 = tf.reduce_mean(
        tf.nn.softmax_cross_entropy_with_logits(logits=out6, labels=tf.one_hot(labels[:, 6], depth=2)))

    loss_list = [loss, loss1, loss2, loss3, loss4, loss5, loss6]
    loss_sum = tf.reduce_sum(loss_list)
    train_op = tf.train.AdamOptimizer(learning_rate=0.00001).minimize(loss_sum, global_step=global_step)
...
def validation():
    ...
    test_feeder = DataIterator(data_dir='./test_data/')
    predict_labels_list = []
    groundtruth = []
    with tf.Session() as sess:
        ...
        try:
            ...
            while not coord.should_stop():
                ...
                labels_batch, predict_labels_batch, acc = sess.run(
                    [endpoints['labels'], endpoints['out_final'], endpoints['accuracy']], feed_dict=feed_dict)
...
def inference(image):
    ...
    temp_image = temp_image.reshape([-1, 30, 280, 1])
        ...
        try:
            while not coord.should_stop():
                ...
                _, loss_val, train_summary, step = sess.run(
                    [endpoints['train_op'], endpoints['loss_sum'], endpoints['merged_summary_op'],
                     endpoints['global_step']], feed_dict=feed_dict)
                ...
                if step % FLAGS.eval_steps == 1:
                    ...
                    accuracy_val, test_summary = sess.run([endpoints['accuracy'], endpoints['merged_summary_op']],
                                                          feed_dict=feed_dict)
                    end_time = time.time()
                    test_writer.add_summary(test_summary, step)
                    logger.info('[test] the step {0} accuracy {1} spend time {2}'.format(step, accuracy_val,
                                                                                         (end_time - start_time)))
